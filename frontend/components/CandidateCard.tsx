...
function getExpertiseBadge(depth: string): { label: string; style: string } {
  switch (depth?.toLowerCase()) {
    case "expert":      return { label: "Expert", style: "bg-green-100 text-green-700 border-green-200" };
    case "proficient":  return { label: "Proficient", style: "bg-blue-100 text-blue-700 border-blue-200" };
    case "developing":  return { label: "Developing", style: "bg-amber-100 text-amber-700 border-amber-200" };
    default:            return { label: "Surface", style: "bg-gray-100 text-gray-500 border-gray-200" };
  }
}
  ...
  return (
    <div className="relative flex-shrink-0" style={{ width: size, height: size }}>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <span className="text-sm font-bold leading-none text-gray-800">{score.toFixed(1)}</span>
      </div>
    </div>
  );
}
...
function ScoreBar({ label, value }: { label: string; value: number }) {
  const color = value >= 8 ? "bg-green-400" : value >= 6 ? "bg-amber-400" : "bg-red-400";
  return (
...
export default function CandidateCard({ candidate, jobId, apiUrl, starred, onStar, listView }: Props) {
  ...
  if (listView) {
    return (
      <>
        <div className="bg-white border border-gray-100 rounded-xl px-5 py-4 hover:shadow-sm transition-shadow flex items-center gap-4">
          {/* Star */}
          <button onClick={onStar} className={`flex-shrink-0 transition-colors ${starred ? "text-amber-400" : "text-gray-200 hover:text-amber-300"}`}>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {candidate.recommended && (
                <span className="px-2 py-0.5 bg-indigo-50 text-indigo-600 text-xs font-semibold rounded-full border border-indigo-100">Recommended</span>
              )}
            ...
            <p className="text-sm text-gray-500 truncate mt-0.5">{candidate.headline}</p>
            <div className="flex items-center gap-3 mt-1 flex-wrap">
              {candidate.location && <span className="text-xs text-gray-400">📍 {candidate.location}</span>}
              {candidate.email && (
                <button onClick={handleCopyEmail} className="text-xs text-indigo-500 hover:text-indigo-700 flex items-center gap-1">
          ...
          <div className="hidden lg:flex flex-wrap gap-1 max-w-xs">
            {visibleSkills.map(s => (
              <span key={s} className="px-2 py-0.5 bg-indigo-50 text-indigo-700 text-xs rounded-md border border-indigo-100 font-medium">{s}</span>
            ))}
            {extraSkills > 0 && <span className="px-2 py-0.5 bg-gray-100 text-gray-500 text-xs rounded-md font-medium">+{extraSkills}</span>}
          </div>
          {/* Score bars */}
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            <button onClick={() => setShowModal(true)}
              className="text-xs px-3 py-1.5 rounded-lg border border-indigo-200 bg-indigo-50 text-indigo-700 hover:bg-indigo-100 transition-colors">
              Interview
            </button>
          </div>
        </div>
  ...
  return (
    <>
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow flex flex-col overflow-hidden">
        ...
        <div className="px-5 pt-5 pb-3">
          <div className="flex items-start justify-between gap-3">
            <div className="flex-1 min-w-0">
              ...
              {candidate.headline && (
                <p className="text-sm text-gray-500 mt-0.5 leading-snug line-clamp-2">{candidate.headline}</p>
              )}
            </div>
            <div className="flex items-center gap-2">
              {/* Star button */}
              <button onClick={onStar} className={`transition-colors ${starred ? "text-amber-400" : "text-gray-200 hover:text-amber-300"}`}>
                <svg className="w-5 h-5" fill={starred ? "currentColor" : "none"} viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" />
                </svg>
              </button>
          ...
          <div className="flex items-center gap-2 flex-wrap mt-2">
            ...
            {candidate.experience_years != null && (
              <span className="text-xs text-gray-400">{candidate.experience_years}y exp</span>
            )}
          </div>
          ...
          {candidate.email && (
            <button onClick={handleCopyEmail}
              className="mt-2 flex items-center gap-1.5 text-xs text-indigo-500 hover:text-indigo-700 transition-colors">
        ...
        {/* Score bars */}
        <div className="px-5 py-2.5 bg-gray-50 border-y border-gray-100 space-y-1.5">
          <ScoreBar label="Technical" value={candidate.technical_score} />
          <ScoreBar label="Culture" value={candidate.culture_score} />
        </div>
        ...
        <div className="px-5 py-3">
          <div className="flex flex-wrap gap-1.5">
            {visibleSkills.map((skill) => (
              <span key={skill} className="px-2 py-0.5 bg-indigo-50 text-indigo-700 text-xs rounded-md border border-indigo-100 font-medium">
                {skill}
              </span>
            ))}
            {extraSkills > 0 && (
              <span className="px-2 py-0.5 bg-gray-100 text-gray-500 text-xs rounded-md font-medium">+{extraSkills} more</span>
            )}
          </div>
        </div>
        ...
        {candidate.notable_work && (
          <div className="px-5 pb-3">
            <p className="text-xs text-gray-500 leading-relaxed">
              <span className="font-semibold text-gray-700">Notable: </span>
              {candidate.notable_work}
            </p>
          </div>
        )}
        ...
        {candidate.scoring_rationale && (
          <div className="px-5 pb-3">
            <p className={`text-xs text-gray-600 leading-relaxed ${showFullRationale ? "" : "line-clamp-2"}`}>
              {candidate.scoring_rationale}
            </p>
          ...
          <div className="border-t border-gray-100">
            <button onClick={() => setOutreachOpen(!outreachOpen)}
              ...
              <span className="flex items-center gap-2">
                ...
                {candidate.outreach_channel && (
                  <span className="text-xs text-gray-400 font-normal">via {candidate.outreach_channel}</span>
                )}
              </span>
              <svg className={`w-4 h-4 text-gray-400 transition-transform ${outreachOpen ? "rotate-180" : ""}`}
            ...
            {outreachOpen && (
              <div className="px-5 pb-4 space-y-2">
                ...
                {candidate.outreach_message && (
                  <div className="bg-gray-50 rounded-lg px-3 py-2">
                    <p className="text-sm text-gray-700 whitespace-pre-wrap leading-relaxed">{candidate.outreach_message}</p>
                  </div>
                )}
                <button onClick={handleCopy}
                  ...
                  {copied ? (
                    <><svg className="w-3.5 h-3.5 text-green-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" /></svg><span className="text-green-600">Copied!</span></>
                  ) : (
